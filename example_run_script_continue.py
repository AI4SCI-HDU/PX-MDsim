...
while n_clink <= max_links and p_set < len(dist):
    ...
    if pair:
        ...
        dpc = round(Sys.atom_count()["DPC_bonds"], 3)
        if bondlog:
            with open(bondlog, 'a') as f:
                f.write(
...
Unreacted_TMA = Sys.remove_residue('TMC')
Unreacted_PIP = Sys.remove_residue('DAP')
logger_script.info(f"Found {Unreacted_TMA} unreacted TMA monomers.")
logger_script.info(f"Found {Unreacted_PIP} unreacted PIP monomers.")
