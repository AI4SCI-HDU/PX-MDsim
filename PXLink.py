...
# This is the main script of PXLink, which defines a class that
...
# doi.org/10.1016/j.cpc.2023.108840
...
class GromacsSys:
    ...
    def __init__(self,
        ...
        self.delta_charge_C = [0, 0, 0]
        self.delta_charge_N = [0, 0, 0]
    ...
    def mark_atoms(self, nofreeze=False):
        ...
        1. Find all hydroxyl oxygen (OG311) and amine nitrogen (A_N)
        ...
        # Find hydroxyl oxygen O_H and amine nitrogen A_N
        O_atoms = self.atoms.loc[self.atoms['type'] == 'H_O']['nr'].tolist()
        N_atoms = self.atoms.loc[self.atoms['type'] == 'A_N']['nr'].tolist()
        ...
        for i in O_atoms:
            ...
            # Find the two atoms connected with H_O (which are carboxyl C and H),
            ...
            alpha_C = self.atoms.loc[
                ...
                & (self.atoms['type'] == 'C_R')]['nr'].tolist()[0]
            charge_list = [
                carboxyl_C,
                self.atoms.loc[(self.atoms['nr'].isin(hydroxyl_C_neighbor)) & (
                    self.atoms['type'] == 'C_O')]['nr'].tolist()[0], alpha_C
            ]
    ...
    def add_bond(self, newbond):
        ...
        atomA = newbond[0]
        atomB = newbond[1]
        if (self.atoms.loc[self.atoms['nr'] == atomA]['type'].tolist()[0] != 'C_C') or\
                (self.atoms.loc[self.atoms['nr'] == atomB]['type'].tolist()[0] != 'A_N'):
            logging.error(
        ...
        self.atoms.loc[self.atoms['nr'] == atomA, 'type'] = 'N_C'
        self.atoms.loc[self.atoms['nr'] == atomB, 'type'] = 'N_N'
        ...
        self.atoms.loc[self.atoms['nr'] == H_change, 'type'] = 'H_H'
        ...
        if operation == 'opt':
            ...
            if verbose:
                if not self.old_gmx:
                    ...
                    sp.check_call('gmx mdrun -v -deffnm ' + tpr_name  + ' -ntmpi 1 -ntomp 8',
                                  shell=True)
                else:
                    logging.info(
                        f"Running geometry optimization: mdrun -v -deffnm {tpr_name}"
                    )
                    sp.check_call('mdrun -v -deffnm ' + tpr_name  + ' -ntmpi 1 -ntomp 8', shell=True)
            else:
                if not self.old_gmx:
                    logging.info(
                        f"Running geometry optimization: gmx mdrun -deffnm {tpr_name}"
                    )
                    sp.check_call('gmx mdrun -deffnm ' + tpr_name  + ' -ntmpi 1 -ntomp 8', shell=True)
                else:
                    logging.info(
                        f"Running geometry optimization: mdrun -deffnm {tpr_name}"
                    )
                    sp.check_call('mdrun -deffnm ' + tpr_name  + ' -ntmpi 1 -ntomp 8', shell=True)
    ...
    def atom_count(self,
        ...
        for name in ['N_C', 'N_N', 'C_C', 'A_N', 'H_O']:
        ...
        if dpc_bonds:
            dpc.update({
                "DPC_bonds": (count_atom['N_N'] /
                              (count_atom['N_N'] + count_atom['A_N'] +
                               count_atom['C_C']))
            })
        if dpc_bonds_appr:
            dpc.update({
        ...
        if dpc_NO_ratio:
            no_ratio = (count_atom['N_N'] + count_atom['A_N']) / (
                count_atom['C_O'] + count_atom['H_O'])
            dpc.update({
                "DPC_crosslinked_portion(NO_ratio)":
                (4 * no_ratio - 2) / (1 + no_ratio)
            })
        if dpc_OO_ratio:
            oo_ratio = count_atom['H_O'] / count_atom['C_O']
            dpc.update(
                {"DPC_crosslinked_portion(Oxygen_ratio)": 1 - 3 * oo_ratio})
        if dpc_carboxyl_ratio:
            cx_ratio = count_atom['N_C'] / (count_atom['C_C'] +
                                              count_atom['N_C'])
        ...
        if log_details:
            ...
            total_C = count_atom['C_R'] + count_atom['C_C'] + count_atom[
                'N_C'] + count_atom['A_R']
            total_N = count_atom['N_N'] + count_atom['A_N']
            total_H = count_atom['H_H'] + count_atom['A_H'] + count_atom[
                'HGR61']
            total_O = count_atom['C_O'] + count_atom['H_O']
            mass_atoms = [
